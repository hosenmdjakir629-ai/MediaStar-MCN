...
const StatCard: React.FC<StatCardProps> = ({ title, value, change, trend, icon: Icon, color }) => {
  ...
  return (
    <div className={`bg-orbit-800/40 backdrop-blur-md rounded-2xl p-5 border border-white/5 hover:bg-orbit-800/60 transition-all duration-300 relative overflow-hidden group shadow-xl ${colorClass.split(' ').pop()}`}>
      {/* Ambient Glow */}
      ...
      <div className="flex justify-between items-start mb-3 relative z-10">
        <div className={`p-2.5 rounded-xl ${colorClass.split(' ').slice(0, 2).join(' ')}`}>
          <Icon size={20} />
        </div>
